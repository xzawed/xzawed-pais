...
const CONSUMER_GROUP = 'designer-consumers'
const CONSUMER_NAME = 'designer-1'
...
export class Consumer {
  ...
  constructor(
    ...
    private readonly onMessage: (msg: ManagerToDesignerMessage) => Promise<void>,
    private readonly sleep: (ms: number) => Promise<void> = (ms) =>
      new Promise((r) => setTimeout(r, ms))
  ) {}

  async start(sessionId: string): Promise<void> {
    const stream = `manager:to-designer:${sessionId}`
  ...
  private async processMessages(stream: string, messages: [string, string[]][]) {
    for (const [msgId, fields] of messages) {
      ...
      const parsed = ManagerToDesignerMessageSchema.safeParse(JSON.parse(raw))
      if (!parsed.success) {
