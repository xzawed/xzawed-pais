...
description: PR 생성 전 체크리스트 자동 실행 — build/test/audit/cpd/E2E선택자/i18n/Dockerfile/위험신호 순차 검증
---
...
프로젝트 루트(`git rev-parse --show-toplevel`)에서:
```
...
앞의 7단계는 전부 **기계가 잡을 수 있는 것**만 본다. 이 단계는 tsc·테스트·CPD가 구조적으로 못 잡는 부류를 정산한다 — 서비스 간 계약 드리프트, fail-open/closed 반전, 플래그 배선 비대칭, 테넌트 경계, 마이그레이션 멱등성, 의존성 major 범프.

브랜치 전체 diff에 대해 신호를 재수집한다(커밋 단위로만 보면 여러 커밋에 흩어진 변경을 놓친다):

```
git diff origin/master...HEAD > /tmp/pr-diff.txt
```

`.claude/hooks/grok-risk-signal.mjs` 의 `SIGNALS` 목록과 동일한 기준으로 이 diff를 검사한다. 감지된 신호마다 **둘 중 하나**를 반드시 기록한다:

1. **반증 완료** — `/grok-verify` 로 독립 검증했고 그 결과가 무엇이었는지. 명령 로그가 근거로 있어야 한다.
2. **명시적 사유** — 왜 이 신호가 이 변경에는 해당하지 않는지. "괜찮아 보인다"는 사유가 아니다. 어떤 코드 경로가 존재하지 않는지, 어떤 테스트가 이미 덮는지를 적는다.

**둘 다 없으면 해당 신호는 미해결이다.** 미해결 신호가 있으면 PR 본문에 그대로 적어 리뷰어가 볼 수 있게 한다. 조용히 넘기지 않는다 — 그게 이 저장소가 M8(무음 통과 금지)로 부르는 실패 유형이다.

신호가 하나도 없으면 `해당 없음`으로 기록하고 넘어간다.
...
[1/8] 빌드       ✅ PASS  /  ❌ FAIL — <오류 요약>
[2/8] 테스트     ✅ PASS (443건)  /  ❌ FAIL — <실패 테스트명>
[3/8] Audit      ✅ PASS  /  ❌ FAIL — <취약점 수>
[4/8] CPD        ✅ PASS (0 clones)  /  ❌ FAIL — <중복 파일 목록>
[5/8] E2E선택자  ✅ PASS  /  ⚠️ WARN — <금지 패턴 위치>
[6/8] i18n 키    ✅ PASS  /  ❌ FAIL — <누락 키>
[7/8] Dockerfile ✅ PASS  /  ⚠️ WARN — <누락 규칙>
[8/8] 위험 신호  ✅ 해당 없음  /  ✅ 정산 완료 (<신호>: 반증|사유)  /  ⚠️ 미해결 — <신호 목록>

전체: <N>/8 통과
PR 생성 가능: YES / NO (빌드·테스트·Audit·i18n 실패 시 NO)
미해결 위험 신호: 없음 / <목록> ← 있으면 PR 본문에 명시
```

FAIL 항목이 있으면 각 항목의 수정 방법을 구체적으로 제시한다.

미해결 위험 신호는 PR 생성을 막지 않는다. 다만 **PR 본문에 반드시 드러내야 한다** — 판단은 리뷰어가 하되, 판단할 재료를 숨기지 않는 것이 이 단계의 목적이다.
