...
ipcMain.handle('token:clear', (): void => {
  ...
  const rPath = getRefreshTokenPath()
  if (existsSync(rPath)) {
    try { unlinkSync(rPath) } catch { /* ignore */ }
  }
})

ipcMain.handle('refresh-token:get', (): string | null => {
  const path = getRefreshTokenPath()
  if (!existsSync(path)) return null
  try {
    const encrypted = readFileSync(path)
    return safeStorage.isEncryptionAvailable()
...
ipcMain.handle('refresh-token:set', (_e, token: string): void => {
  const dir = app.getPath('userData')
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
  const data = safeStorage.isEncryptionAvailable()
    ? safeStorage.encryptString(token)
    : Buffer.from(token, 'utf-8')
  writeFileSync(getRefreshTokenPath(), data)
})
