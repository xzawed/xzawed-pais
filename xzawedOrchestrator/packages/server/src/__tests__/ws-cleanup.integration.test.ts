...
async function startServer(graceMs = 100): Promise<{ app: FastifyInstance; port: number }> {
  const app = await buildServer(
    { port: 0, redisUrl: 'redis://127.0.0.1:6380', managerUrl: 'http://localhost:3001', claudeMode: 'cli', mode: 'local', auth: 'none', claudeModel: 'test', serveWeb: false, allowedOrigins: [], trustProxy: false, wsCleanupGraceMs: graceMs },
