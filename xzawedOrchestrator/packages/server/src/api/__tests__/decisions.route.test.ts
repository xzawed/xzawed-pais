...
describe('decisionsRoutes 프로젝트 소유권 게이트 (G11 Slice 0·IDOR 폐색)', () => {
  ...
  it('G11 Slice 2: orgId claim 토큰 → org 경계 게이트로 미소속 404(Manager 미호출)', async () => {
    const fetchMock = vi.fn()
    vi.stubGlobal('fetch', fetchMock)
    // orgId claim이 있으면 projectOwnershipPreHandler가 findByIdAndOrg 경로를 탄다(mockPool [] → 미소속 404).
    const orgToken = issueAccessToken({ sub: 'po-7', email: 'a@b.c', displayName: null, orgId: 'org-7' }, USER_SECRET)
    const app = await buildOwned([])
    const res = await app.inject({ method: 'POST', url: '/projects/other-org-proj/decisions/r1/decision', headers: { authorization: `Bearer ${orgToken}` }, payload: { choice: 'approve' } })
    expect(res.statusCode).toBe(404)
    expect(fetchMock).not.toHaveBeenCalled()
    await app.close()
  })
