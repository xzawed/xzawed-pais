...
│       │   ├── knowledge.route.ts    # GET|PATCH|DELETE /projects/:id/knowledge[/:id] — Manager 위키 프록시(상태코드 pass-through, GET 실패 시 빈 목록 폴백, 비인증 read-only)
...
└── app/        # Electron 앱 (React 19 + Zustand + electron-vite)
    ...
    └── src/renderer/src/
        ...
        └── components/
            ...
            │   └── UiSpecPreview.tsx  # 승인 게이트용 UISpec 읽기 전용 데모 렌더러(form 필드/mockup·progress content; 상호작용 없음)
            ...
            ├── ChatView.tsx           # AgentTimelineCard + PipelineStrip + UserBubble 통합; pendingInfoRequest.approval 시 승인 카드(승인/수정/중단 + rememberAuto + 지식성 단계 한정 위키저장 체크박스) 렌더 → postUiAction으로 결정 JSON 전송
...
- **Vitest 3** + `vitest.config.ts` `projects` API — `unit` (node) + `browser` (playwright/chromium) 두 프로젝트 분리
  ...
  - 총 `pnpm test`: **181건** (app) + **376건** (server, Redis/DB 없으면 일부 skip) + **74건** (ui, jsdom) = **~631건**
...
- 버튼: 승인(`approve`) / 수정(`revise`, 피드백 입력 필수) / 중단(`abort`). 결정은 JSON으로 직렬화해 `postUiAction()` → `POST /sessions/:id/ui-actions` → Manager에 `info_response{answer}` 발행.
  - `approve` 시 `{decision, rememberAuto, saveToWiki}`, `revise` 시 `{decision, feedback}`, `abort` 시 `{decision}` 전송.
...
- `stage === 'design_ui'` + uiSpec 수신 시 `UiSpecPreview`로 디자인 산출물을 읽기 전용 미리보기.
