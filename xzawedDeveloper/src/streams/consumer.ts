...
const CONSUMER_GROUP = 'developer-consumers'
const CONSUMER_NAME = 'developer-1'
...
export class Consumer {
  ...
  constructor(
    ...
    private readonly onMessage: (msg: ManagerToDeveloperMessage) => Promise<void>,
    private readonly sleep: (ms: number) => Promise<void> = (ms) =>
      new Promise((r) => setTimeout(r, ms))
  ) {}

  async start(sessionId: string): Promise<void> {
    const stream = `manager:to-developer:${sessionId}`
  ...
  private async processMessages(stream: string, messages: [string, string[]][]) {
    for (const [msgId, fields] of messages) {
      ...
      const parsed = ManagerToDeveloperMessageSchema.safeParse(JSON.parse(raw))
      if (!parsed.success) {
