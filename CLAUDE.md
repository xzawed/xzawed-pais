...
| 서비스 | 포트 | 상태 | 역할 | 상세 |
|---|---|---|---|---|
| xzawedOrchestrator | 3000 | ✅ ~631건(+E2E 110건) | 사용자 지시 수신·정제 후 Manager에 전달; GitHub·MCP·Plugin 통합 Electron UI; 승인 게이트 UI·도메인 위키 뷰어; 인증(JWT·Rate Limit·Refresh); i18n(ko/en/ja) | [CLAUDE.md](xzawedOrchestrator/CLAUDE.md) |
| xzawedManager | 3001 | ✅ 277/280 | Claude tool-calling 루프, 하위 에이전트 디스패치; 승인 게이트·도메인 위키·AgentQuery 교차질의 라우팅; github-ops·deploy-project ToolHandler; WatcherEventConsumer | [CLAUDE.md](xzawedManager/CLAUDE.md) |
| xzawedShared | — | ✅ 70/70 | 에이전트 서비스 공통 BaseConsumer + validateWorkspaceRoot + resolveWorkspaceRoot + SessionDispatcher + 협업 헬퍼 + 도메인 위키 주입 포매터 라이브러리 (@xzawed/agent-streams) | [CLAUDE.md](xzawedShared/CLAUDE.md) |
| xzawedPlanner | 3002 | ✅ 92/92 | intent → 실행 가능한 Step[] 분해; AgentQuery 교차질의 발생·응답 | [CLAUDE.md](xzawedPlanner/CLAUDE.md) |
...
| xzawedTester | 3005 | ✅ 61/61 | 테스트 실행·분석; AgentQuery 응답 | [CLAUDE.md](xzawedTester/CLAUDE.md) |
| xzawedBuilder | 3006 | ✅ 132/132 | 프로젝트 빌드 감지·실행; AgentQuery 응답 | [CLAUDE.md](xzawedBuilder/CLAUDE.md) |
...
- **라우팅**: `xzawedManager runner.ts`의 `processToolUseBlocks`가 `AgentQueryError`를 catch → `resolveAgentTool(err.to)`로 대상 도구 해석 → 대상 핸들러를 `{ query, queryKind }`로 실행 → 응답을 `clarificationContext`로 원 에이전트 재실행.

### (b) 승인 게이트 — Human-in-the-loop

에이전트 디스패치 결과를 PO(사용자)가 승인/수정/중단하는 코드 강제 게이트.

- **순수 모듈**: `xzawedManager gates/approval-gate.ts` — `effectiveMode`(override·default 해석)·`isGatedTool`·`parseDecision`(approve/revise/abort, fail-open)·`GATED_TOOLS`·`DEPLOY_TOOLS`(deploy_project는 **항상 manual**, auto override 무시)·`KNOWLEDGE_BEARING_STAGES`(plan/design/develop/security).
- **runner 훅**: `applyApprovalGate` 루프 — manual이면 `info_request`(payload.approval) 발행 후 대기 → approve(`rememberAuto`면 이후 단계 자동 승인, `saveToWiki`면 결정 요약 위키 저장)·revise(피드백으로 재실행, `MAX_GATE_REVISES` 상한)·abort(`GateAbortError`).
- **승인 UI**: Orchestrator `ChatView.tsx` 승인 카드 + `POST /sessions/:id/ui-actions`(결정 → `info_response` 발행) + `lib/api.ts` `postUiAction`. design_ui 데모는 카드에 인라인 렌더.
