...
| xzawedManager | 3001 | ✅ 375/380(5 skip) | Claude tool-calling 루프, 하위 에이전트 디스패치; 승인 게이트(재실행 산출물 포함·전역 모드·**fail-safe**: 파싱 불가·미지 응답은 자동 승인 금지·사람 재요청)·도메인 위키(쓰기 인증)·AgentQuery 교차질의 라우팅·**세션 이벤트소싱+트랜잭셔널 아웃박스**(append-only 진실원천·replay 복원, `EVENT_SOURCED_SESSION` flag 가역); github-ops·deploy-project ToolHandler; WatcherEventConsumer | [CLAUDE.md](xzawedManager/CLAUDE.md) |
| xzawedShared | — | ✅ 137/137 | 에이전트 서비스 공통 BaseConsumer(**바운드 재시도+DLQ 격리·멱등 소비 dedup·전송은 EventBus 위임**) + **EventBus 전송 추상화(RedisEventBus: 발행+소비 포트)** + validateWorkspaceRoot + resolveWorkspaceRoot + SessionDispatcher + 협업 헬퍼 + 도메인 위키 주입 포매터 라이브러리 (@xzawed/agent-streams) | [CLAUDE.md](xzawedShared/CLAUDE.md) |
