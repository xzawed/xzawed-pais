...
| 서비스 | 포트 | 상태 | 역할 | 상세 |
|---|---|---|---|---|
| xzawedOrchestrator | 3000 | ✅ 73/73 | 사용자 지시 수신·정제 후 Manager에 전달; GitHub·MCP·Plugin 통합 Electron UI | [CLAUDE.md](xzawedOrchestrator/CLAUDE.md) |
| xzawedManager | 3001 | ✅ 60/60 | Claude tool-calling 루프, 하위 에이전트 디스패치; github-ops ToolHandler | [CLAUDE.md](xzawedManager/CLAUDE.md) |
...
1. **대시보드 스크린샷 먼저** — `https://sonarcloud.io/project/overview?id=xzawed_xzawed-pais` → Duplications 탭에서 정확한 파일과 줄 수 확인. 코드 분석만으로는 특정 불가.
2. CPD 실패면 해당 파일 내 반복 패턴을 헬퍼 함수로 추출
3. `sonar.cpd.exclusions`는 **Automatic Analysis에서 동작하지 않음** — `sonar.exclusions` 사용

### sonar-project.properties 설정 원칙

```properties
# 테스트 파일 제외: sonar.exclusions만 동작 (sonar.cpd.exclusions 무효)
sonar.exclusions=**/*.test.ts,**/*.spec.ts,**/__tests__/**
sonar.cpd.exclusions=**/*.test.ts,**/*.spec.ts,**/__tests__/**  # 혹시 모를 스캐너 모드 대비
```
