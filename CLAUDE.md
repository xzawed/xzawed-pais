...
| xzawedPlanner | 3002 | ✅ 48/48 | intent → 실행 가능한 Step[] 분해; AgentQuery 교차질의 발생·응답 | [CLAUDE.md](xzawedPlanner/CLAUDE.md) |
| xzawedDeveloper | 3003 | ✅ 64/64 | 코드 생성·수정, 파일 I/O; **D5 모델 라우팅 소비**(`generateChanges(model?)`=`model ?? this.model`·`payload.model` 스레딩 — Manager 워커가 주입한 WP별 모델 id로 호출, 미주입 시 `CLAUDE_MODEL` 폴백·`payload.model`은 `collaborationPayloadFields`의 `model?: z.string()`로 보존·합성 스키마 회귀 가드 #333) | [CLAUDE.md](xzawedDeveloper/CLAUDE.md) |
| xzawedDesigner | 3004 | ✅ 43/43 | UI 컴포넌트 스펙 설계 | [CLAUDE.md](xzawedDesigner/CLAUDE.md) |
| xzawedTester | 3005 | ✅ 65/65 | 테스트 실행·분석; AgentQuery 응답; 도메인 지식 emit | [CLAUDE.md](xzawedTester/CLAUDE.md) |
| xzawedBuilder | 3006 | ✅ 71/71 | 프로젝트 빌드 감지·실행; AgentQuery 응답; 도메인 지식 emit | [CLAUDE.md](xzawedBuilder/CLAUDE.md) |
...
**각 서비스의 `package.json`**(저장소 루트에는 package.json이 없다 — 11개 독립 pnpm 프로젝트다)에 `pnpm.overrides`로 강제 해결:
...
**알림이 재발하면 새 취약점보다 낡은 핀을 먼저 의심한다.** 2026-08-08 36건 중 대부분이 기존 override 핀(`fast-uri@3: ^3.1.4` 등)이 신규 권고 버전에 추월당한 경우였다(#513). 진단은 알림의 `first_patched_version`을 현재 핀과 대조하면 끝난다:

```bash
gh api "repos/xzawed/xzawed-pais/dependabot/alerts?state=open&per_page=100" --paginate \
  --jq '.[] | "\(.dependency.manifest_path)\t\(.dependency.package.name)\t\(.security_vulnerability.vulnerable_version_range)\t->\(.security_vulnerability.first_patched_version.identifier)"' | sort -u
```

**CI audit 게이트는 `pnpm audit --audit-level=moderate`(dev 포함)다.** `--prod`만 보면 devDependency 경유 취약점(electron-builder→js-yaml, postcss→nanoid 등)을 놓친다. Dependabot 알림에 없어도 이 게이트가 막는 항목이 있으므로 로컬에서 같은 명령으로 확인한다.

**`pnpm.auditConfig.ignoreGhsas`는 무시 사유의 만료 여부를 함께 재검증한다.** 2026-08-08에 2건 모두 stale로 확인돼 제거했다 — react-router는 "8.x에서만 수정됨, v8 전환 때 재검토"로 기록돼 있었으나 실제로는 7.18.2에서 패치된 상태였다(#514). 판단은 추정이 아니라 **ignore를 비운 상태로 audit을 실측**해서 한다.
...
- **Dependabot PR의 SonarCloud 실패는 코드 문제가 아니다** — bot PR은 `SONAR_TOKEN`에 접근할 수 없어 구조적으로 실패한다("Not authorized or project not found"). 로컬 일괄 반영 후 사람 브랜치 단일 PR로 우회하거나(#494·#513 방식), 워크플로에서 `github.actor == 'dependabot[bot]'`일 때 Sonar 잡을 skip해야 한다
