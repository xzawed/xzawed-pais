...
| 서비스 | 포트 | 상태 | 역할 | 상세 |
|---|---|---|---|---|
| xzawedOrchestrator | 3000 | ✅ ~708건(+E2E 110건) | 사용자 지시 수신·정제 후 Manager에 전달; GitHub·MCP·Plugin 통합 Electron UI; 승인 게이트 UI(저장 전 summary 편집·전역 게이트 모드)·도메인 위키 뷰어(쓰기 인증); 인증(JWT·Rate Limit·Refresh); i18n(ko/en/ja) | [CLAUDE.md](xzawedOrchestrator/CLAUDE.md) |
| xzawedManager | 3001 | ✅ 293/296 | Claude tool-calling 루프, 하위 에이전트 디스패치; 승인 게이트(재실행 산출물 포함·전역 모드)·도메인 위키(쓰기 인증)·AgentQuery 교차질의 라우팅; github-ops·deploy-project ToolHandler; WatcherEventConsumer | [CLAUDE.md](xzawedManager/CLAUDE.md) |
