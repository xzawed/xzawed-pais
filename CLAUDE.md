...
| xzawedManager | 3001 | ✅ 387/395(8 skip) | Claude tool-calling 루프, 하위 에이전트 디스패치; 승인 게이트(재실행 산출물 포함·전역 모드·**fail-safe**: 파싱 불가·미지 응답은 자동 승인 금지·사람 재요청)·도메인 위키(쓰기 인증)·AgentQuery 교차질의 라우팅·**세션 이벤트소싱+트랜잭셔널 아웃박스**(append-only 진실원천·replay 복원, `EVENT_SOURCED_SESSION` flag 가역)·**Task Graph 영속**(task_graphs 가변 프로젝션+wp_state_log append-only, P1d-3); github-ops·deploy-project ToolHandler; WatcherEventConsumer | [CLAUDE.md](xzawedManager/CLAUDE.md) |
