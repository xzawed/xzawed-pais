...
현재 상태: **구현 완료 (server 917/968 테스트 — 로컬 51 skip, CI는 pg 통합 포함 실행)**
...
- **후속(P5-2b)**: `gate.passed`→deploy_project hard 전제(deploy 게이팅)·사인오프 후 promote 소비. `scope='release'` 사인오프 레코드가 그 진실원천. degraded_release의 reject/fix_reverify/spec_fix는 설계상 no-op(reject=block 유지·saga는 P5-4).
