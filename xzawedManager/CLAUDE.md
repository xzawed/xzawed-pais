...
- **한계(후속)**: golden_refs는 현재 비어 있음(**초안 생성기 없음 — invariants는 F5로 활성화됐으나 golden은 LLM fabricate 불가·N7이라 record-and-freeze 슬라이스 후속**)→사람이 golden 추가 전까지 휴면(conformance와 동일). affected-story 회귀+결합도-냄새→advisory 라우팅은 후속.
