...
- **배선(`server.ts`)**: `EVENT_SOURCED_SESSION` + `DATABASE_URL`이면 `EventStore` 생성 → 시작 시 `replaySessions()` 복원. **`OutboxRelay`는 아웃박스를 쓰는 어떤 flag(`EVENT_SOURCED_SESSION`·`TASK_MANAGER_ENABLED`·`MANAGER_ORACLE_DOR`)라도 켜지면 가동**(아웃박스→Redis 발행은 이벤트소싱과 독립 — 미기동 시 wp.dispatched·oracle.approved 행이 잔류해 디스패치/재디스패치 불발). `closeAll`에서 relay stop.
...
- **계약(additive optional)**: `decompose_request.payload.userContext`(`UserContextSchema` — userId·projectId·workspaceRoot·githubRepo?) → `decomposition.emitted.payload.userContext` → `task_graphs.graph_dag = {workPackages, userContext?}`.
- **스레딩**: sessions.route → `handleDecomposeRequest(..., userContext?, ensureWs)`(trigger try 안에서 `ensureWorkspace` — task_request 경로 대칭·실패 시에도 finally cleanup) → `produceDecomposition(..., userContext?)`(ok·기술 fallback 경로 포함, inconsistent 경로 제외) → `handleDecompositionEmitted`가 `upsertGraph`에 전달.
- **영속·조회(`TaskGraphRepo`)**: `PersistGraphInput.userContext?`(null/undefined면 키 생략)·`StoredGraph.userContext: UserContext | null`. `getGraph`는 **safeParse tolerant** — 레거시 행·손상 데이터는 null(디스패치 경로 보호·워커는 placeholder 폴백 우아한 강등). workPackages는 기존대로 strict.
- **워커 주입(`worker.ts`)**: `AgentExecutor.execute(input, sessionId, userContext?)`(3번째 옵셔널 — `RedisAgentHandler.execute` 시그니처와 일치·2-인자 구현도 구조적 할당 가능). `buildWorkerInput(wp, userContext?)`가 `projectPath = userContext?.workspaceRoot ?? '.'` — builder/tester `validatePath`는 `fs.realpath(projectPath)`를 **에이전트 cwd 기준**으로 해석하므로 절대경로가 cwd 무관 통과(NEW-2의 실체). `handleWpDispatchSignal`이 `stored.userContext`를 입력·execute 양쪽에 전달 → RedisAgentHandler가 `payload.userContext`로 spread → 에이전트 `resolveWorkspaceRoot(payload.userContext, config.workspaceRoot)` 소비.
