...
현재 상태: **구현 완료 (server 954/1019 테스트 — 로컬 65 skip, CI는 pg 통합 포함 실행)**
...
- **한계(후속)**: `decision.expired` 소비자(재에스컬레이션·알림) 부재(B1=발행까지=M8 바)·단일 TTL(타입별 차등 후속)·TTL 연장 없음.
...
MANAGER_DECISION_EXPIRY=        # 선택: 기본 false(B1). true면 결정 요청에 생성 시 expiresAt(TTL) 부여 + DecisionSweeper가 만료 PENDING을 EXPIRED 전이·decision.expired 발행(M8 비-무음). 전제: TASK_MANAGER_ENABLED+DATABASE_URL(Supervisor 배선). off면 expiresAt 미설정·sweep 미배선(회귀 0). 레거시 null 행 제외(소급 만료 0)
MANAGER_DECISION_TTL_HOURS=     # 선택: 기본 72(B1). 결정 TTL(시간). server가 *3_600_000(ms)로 변환해 주입. 사람 대면이라 충분한 시간
MANAGER_DECISION_SWEEP_MS=      # 선택: 기본 60000(B1). 결정 만료 sweep 주기 ms(이미 ms·무변환). 만료는 시간 단위라 분 단위 충분
