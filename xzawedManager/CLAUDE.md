...
- **`buildServer`를 부르는 테스트는 저장소에 하나도 없다 — 그래서 그 안에 직접 쓴 판정은 검증되지 않는다.** DB·Redis·Anthropic 배선을 통째로 끌고 와 테스트가 부르지 못한다. 실측: `server.ts` 라인 **1/236 · 분기 0/396**(주변은 `worker.ts` 97.9% · `verify.ts` 96.5% · `supervisor.ts` 91.5%로, 고립된 구멍 하나이지 일반적 약점이 아니다). **이 구멍은 이미 두 번 결함을 냈다** — (1) 인스턴스 옵션 두 줄이 오래 틀렸다(로거가 `MODE==='local'`이라 **프로덕션에서만 꺼져** `app.log.*` 호출부 65곳이 no-op, `trustProxy`는 `true` 하드코딩), (2) advisory 기동 경고가 **사실과 반대**였다(아래).
