...
현재 상태: **구현 완료 (server 635/659 테스트 — 로컬 24 skip, CI는 pg 통합 21건 포함 656/659 실행)**
...
- **`streams/verify.ts` 순수 코어**: `judgePrimaryResult(tool, result)` — **결과-근거 판정**(run_tests `success && failed===0`·build_project `success`; 판정 전용 minimal Zod·**기본값 없음** — 필드 부재=파싱 실패=fail). `planVerificationChecks(tool)` — **파생 체크 플랜**(develop_code → `['build_project','run_tests']` fail-fast 순서; run_tests/build_project WP는 자기 결과가 이미 ground truth라 이중 실행 회피·design_ui/security_audit는 실행 가능 ground truth 부재(4d) → 빈 플랜). `verifyWp(tool, wp, result, deps)` — ①→② 오케스트레이션, **never-throw**(핸들러 부재·throw·파싱 실패 전부 fail verdict — 불확실=실패).
- **파생 체크 실행**: 워커 `handlers` 맵(server.ts 5종)을 재사용해 `buildWorkerInput(wp, userContext)` 입력으로 builder/tester를 같은 워크스페이스에 실 재호출 — 판정은 LLM 선언이 아니라 실 spawn 실행 결과 필드(N1).
- **워커 통합(`worker.ts`)**: `WorkerDeps.verifyEnabled?`(기본 false). execute 성공 후 verdict fail이면 **완료 미발행** + `wp.verification.failed` 관측 이벤트(`manager:events:{wf}`·reason 500자 클램프·**best-effort** try/catch — 부재한 completion이 load-bearing 신호) 후 outcome `verification_failed`. → lease 만료 → reclaim attempt++ → 상한 초과 ESCALATED(**기존 P1d-5 백스톱이 N5 바운드 재시도·사람 에스컬레이션 담당** — 새 재시도 메커니즘 없음).
- **배선**: `SupervisorConfig.wpVerify?`(optional additive) → `createSupervisor`가 WorkerConsumer deps에 `verifyEnabled` 스레딩. server.ts가 `config.MANAGER_WP_VERIFY` 전달 + 전제(`MANAGER_TASK_WORKER`) 미충족 시 오진 방지 경고.
- **한계(후속)**: 빈 스위트 vacuous pass(테스트 0개 프로젝트의 derived run_tests `failed:0` 통과)는 N8 mutation 게이트(4b 후속)가 봉합. 검증 실패 사유가 reclaim 재실행 입력에 미반영(비정보 재시도 — 4c informed rework). 오라클 GWT 시나리오는 아직 검증 미사용(step-def 컴파일=4b-2).
