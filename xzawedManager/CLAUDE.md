...
현재 상태: **구현 완료 (server 1079/1153 테스트 — 로컬 74 skip, CI는 pg 통합 포함 실행)**
...
- **한계(후속)**: DEGRADED HIGH-risk 사인오프(N2)·reclaim/fix_reverify 경로 게이팅(SAFE 중에도 budget 서킷이 fail-closed 백스톱)·재시작 held 복원/startup 재디스패치 sweep·즉시 tick(게이트 지연 ≤ `MANAGER_MODE_SWEEP_MS`·budget 서킷 즉시 백스톱).
