...
- **advisory 는 통과한 verdict 를 전제한다 — 정상 동작·안정성이 먼저이고 최적화는 그다음이다.** 판정 없는 산출물에는 제안하지 않는다. 정책의 **절반은 구조가 이미 보장한다**: 실패한 verdict는 호출부 `if (gate) return gate`가 막는다. 남는 구멍은 **검증이 꺼져 판정 자체가 없는** 경우 하나이고, `maybeProduceAdvisory` 가드의 `verifyEnabled === true`가 그것을 닫는다 — 즉 그 한 줄이 정책 전부다. **이 경고 문구는 과거에 코드와 반대 방향으로 어긋난 적이 있다**(`runVerifyGate`가 `!verifyEnabled`에 판정 없이 `null`을 반환하는 것을 놓쳤다). 문구를 고칠 때는 `worker.advisory.test.ts`(워커 동작)와 `startup-warnings.test.ts`(경고 문구) **양쪽**을 함께 본다
