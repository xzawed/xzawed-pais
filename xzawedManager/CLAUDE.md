...
현재 상태: **구현 완료 (server 797/837 테스트 — 로컬 40 skip, CI는 pg 통합 포함 실행)**
...
- **`streams/conformance.ts`**: `selectConformanceTestFiles`를 `selectAuthoredTestFiles(artifacts, dir, wpId)`로 일반화(conformance는 CONFORMANCE_DIR 위임·API 보존)·`IMPACT_DIR='.xzawed/impact'`·`buildGoldenDiffAuthorPlan(wp, goldens)`·`ImpactOracleStore` 포트.
- **`streams/verify.ts`**: `runAuthoredCheck<T>`(가드)+`executeAuthoredTest`(author→run)+`runConformanceCheck`/`runImpactCheck` 래퍼. `verifyWp`가 develop_code에서 conformance→impact 순서 **hard-AND**. never-throw fail-closed.
