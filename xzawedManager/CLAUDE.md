...
현재 상태: **구현 완료 (server 700/730 테스트 — 로컬 30 skip, CI는 pg 통합 27건 포함 727/730 실행)**
...
└── server/
    └── src/
        ...
        ├── claude/runner.ts    # Claude tool-calling 루프 (승인 게이트·위키 주입/저장·AgentQuery 라우팅·**§13 budget 서킷**: 호출 전 check(fail-closed)·호출 후 record·트립 시 onTrip 알림, optional 주입·미주입 시 회귀 0)
