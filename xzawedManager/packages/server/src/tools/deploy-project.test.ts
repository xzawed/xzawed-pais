...
const denyGate: DeployGatePort = { checkDeploy: async () => ({ allowed: false, reason: '게이트 차단 사유 X' }) }
const allowGate: DeployGatePort = { checkDeploy: async () => ({ allowed: true }) }

describe('deploy_project 릴리스 게이트', () => {
  it('blocked verdict → execute가 throw(GitHub 호출 전)', async () => {
    const h = createDeployProjectHandler('tok', 'redis://x', denyGate)
    await expect(
      h.execute(
