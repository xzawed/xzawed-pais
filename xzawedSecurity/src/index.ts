...
async function main() {
  ...
  const dispatcher = new SessionDispatcher(
    gatewayRedis,
    'manager:to-security:sessions',
    'security-session-dispatcher',
    (_sessionId: string) => {
      ...
      return new Consumer(sessionRedis, (msg) => security.handle(msg))
    },
  )

  const server = createServer()
  await server.listen({ port: config.port, host: '0.0.0.0' })
  console.log(`xzawedSecurity listening on :${config.port}`)
