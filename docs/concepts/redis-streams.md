[홈](../README.md) > [개념](.) > Redis Streams 메시징
...
## SessionDispatcher 게이트웨이 패턴

Phase 3(PR #122) 이후 7개 서브에이전트는 `SessionDispatcher`를 사용한다.

**게이트웨이 스트림:** `manager:to-{agent}:sessions`
- Manager가 에이전트를 처음 호출할 때 이 스트림에 세션 알림을 발행
- `SessionDispatcher`가 이 알림을 수신하면 per-session consumer를 동적으로 생성

**퍼세션 스트림:** `manager:to-{agent}:{sessionId}`
- 세션별로 독립된 스트림에서 메시지를 소비
- 동시 세션 간 메시지가 완전히 격리됨
