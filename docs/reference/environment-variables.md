...
| 변수 | 기본값 | 필수 | 가능한 값 | 설명 |
|------|--------|------|-----------|------|
| `CLAUDE_MODE` | `api` | 아니오 | `cli`, `api`, `remote` | Claude 실행 방식 선택 |
...
`AUTH=jwt` 로 설정하거나 사용자 인증 기능을 사용할 때 필요한 변수입니다.

| 변수 | 기본값 | 필수 | 설명 |
|------|--------|------|------|
| `SERVICE_JWT_SECRET` | — | `AUTH=jwt` 시 필수 (32자 이상) | 서비스 간 JWT 서명 시크릿. 32자 미만이면 서버 기동 시 에러 |
| `USER_JWT_SECRET` | — | 사용자 인증 사용 시 필수 | 사용자 access token 서명 시크릿. `DATABASE_URL`과 함께 설정 시 `/auth/*`·`/projects` 엔드포인트 활성화 |
...
| 변수 | 기본값 | 필수 | 설명 |
|------|--------|------|------|
| `DATABASE_URL` | — | 사용자 인증·프로젝트 기능 사용 시 필수 | PostgreSQL 연결 URL (`postgres://user:pass@host:5432/db`). 설정 시 세션·메시지가 DB에 영속됨 |
...
| 변수 | 기본값 | 필수 | 설명 |
|------|--------|------|------|
| `GITHUB_TOKEN_ENCRYPTION_KEY` | — | GitHub PAT 저장 기능 사용 시 필수 | AES-256-GCM 암호화 키 (32바이트 base64). `PUT /projects/:id/github-token` 엔드포인트 사용 시 필요 |
...
| 변수 | 기본값 | 필수 | 설명 |
|------|--------|------|------|
| `SERVE_WEB` | `false` | 아니오 | `true`로 설정하면 서버가 빌드된 웹 클라이언트(`web/dist/`)를 정적 파일로 서빙 |
| `ALLOWED_ORIGINS` | — | 아니오 | CORS 허용 오리진 목록 (쉼표 구분). `MODE=remote`이고 빈 값이면 CORS 전체 차단 |
...
# ===== Claude 실행 모드 =====
# api: Anthropic API 직접 호출 (기본값, API 키 필요)
# cli: 로컬 Claude Code CLI 사용 (Claude 구독 필요)
...
# ===== 인증·데이터베이스 (AUTH=jwt 또는 사용자 인증 사용 시) =====
...
| `AUTH=jwt`이고 `SERVICE_JWT_SECRET` 없거나 32자 미만 | `SERVICE_JWT_SECRET must be at least 32 characters when AUTH=jwt` |
