...
- **security 채널 (4d)** (`streams/verify.ts`, P4) — 검증 correctness 게이트의 **다섯째 차단 채널**(spec §9 보안 렌즈). develop_code WP 산출물에 **결정론 SAST(정적 분석 + npm audit)를 직접 실행**해(author→run 우회 없이 `security_audit` 1회 호출·즉시 작동·휴면 아님) `source∈{static,deps}` ∧ `severity≥floor` findings가 있으면 **blocking**. **LLM(`source:'llm'`) findings는 제외(N6 — 비결정론 차단 금지)**. `runSecurityCheck`가 채널 루프 hard-AND 마지막 append(`[conformance, impact, property, mutation, security]`). xzawedSecurity `SecurityIssue.source`(static/deps/llm) 태그를 소비(`tools/security-audit.ts` outputSchema에 `source` 통과). **oracle 미소비**. floor는 `MANAGER_WP_SECURITY_MIN_SEVERITY`(기본 high). `MANAGER_WP_SECURITY` flag(전제 `MANAGER_TASK_WORKER`+`MANAGER_WP_VERIFY`)·off면 회귀 0. 잔여: deps 분석기 fail-open·static 규칙 5종으로 좁음·STRIDE author→run·Tester 적대 측면·design_ui/security_audit WP 자기검증.
...
│   ├── verify.ts              # 검증 게이트 — correctness + conformance + impact + property + mutation + security 채널 (P4b-1·P4b-2·P4 N8·property·mutation·4d)
...
| `MANAGER_WP_SECURITY` | security 채널(4d) — develop_code WP 검증 시 결정론 SAST(static+npm audit)를 산출물에 직접 실행→`source∈{static,deps}`∧`severity≥floor`면 blocking(LLM findings 제외·N6). oracle 미소비·즉시 작동. 전제: `MANAGER_WP_VERIFY`(`MANAGER_TASK_WORKER` 포함), 가시성 360s↑ 권장 | P4 |
| `MANAGER_WP_SECURITY_MIN_SEVERITY` | security 채널 차단 최소 심각도 floor(low/medium/high/critical). 기본 high. 불량값은 high 폴백 | P4 |
