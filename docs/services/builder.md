...
`detectBuildInfo(path)`: `projectPath`에서 `workspaceRoot`까지 상향 탐색 → 우선순위:
1. `Cargo.toml` → `cargo build`
2. `Makefile` → `make`
3. `package.json` (의존성 기반) → `pnpm build` / `npm run build` / `tsc`
4. `go.mod` → `go build ./...`

### 전처리 단계 (builder.ts)

1. `stripPackageManagerField()`: Corepack 충돌 방지를 위해 빌드 전 `package.json`의 `packageManager` 필드 임시 제거
2. `runPreInstall()`: `node_modules` 없을 때만 실행

### 스트리밍 출력

stdout/stderr 청크를 버퍼링 없이 즉시 `build_progress`로 발행.
