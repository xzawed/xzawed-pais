...
- `server.ts`: oracleStore 생성 OR-조건에 `|| config.MANAGER_WP_PROPERTY` 추가(**공유 OracleRepo 재사용**·2번째 인스턴스 금지). `createSupervisor` config에 `wpProperty: config.MANAGER_WP_PROPERTY` 전달. 오진 방지 경고: ①`MANAGER_WP_VERIFY` off(verifyWp 미경유 무음 no-op) ②oracleStore 부재(항상 skip) ③`MANAGER_LEASE_VISIBILITY_MS` 하한 — conformance+impact+property 동시 시 WP당 에이전트 호출 최대 ~9단계 → 가시성 상향 권장.
