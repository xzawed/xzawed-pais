...
### D2. attempt 리셋(0) on 재진입
escalated WP 재오픈 시 `attempt=0`으로 리셋 — 사람 승인이 **새 재시도 풀**을 부여. escalate는 `attempt+1≥maxAttempts`에서 발화하므로, 리셋 없이 재오픈하면 즉시 또 escalate(무의미). 무한 루프는 사람 게이트(매 escalation마다 새 결정 필요)가 차단 — 자동 재시도가 아니다(N5).
...
```typescript
async reopenLease(workflowId, wpId, visibilityMs): Promise<{ status:'reopened'; eventId; seq } | { status:'skipped' }>
```
`transition`(기존 공통 tx) 재사용: `UPDATE wp_leases SET status='active', attempt=0, expires_at=now+visibilityMs, event_id=$env, updated_at=NOW() WHERE wf,wp AND status='escalated' RETURNING wp_id`(escalated→active 단방향 가드·0행→skip) + `appendWpEvent`(eventType `WP_DISPATCHED_EVENT`·fromState `ESCALATED_STATE`·toState `DISPATCHED_STATE`·reason `'human_fix_reverify'`·attempt 0). 멱등키는 attempt 0 + event_type(appendWpEvent가 분리). `wpEnvelope(wf, wpId, 0, now)`.
...
- `fix_reverify`: `req = await decisionStore.getRequest(requestId)`; `req?.wpId`면 `r = await leaseStore.reopenLease(req.workflowId, req.wpId, visibilityMs)`; `r.status==='reopened'`면 `publishDispatchSignal(publish, req.workflowId, req.wpId, 0, now)`. req/wpId 부재·skip이면 no-op.
