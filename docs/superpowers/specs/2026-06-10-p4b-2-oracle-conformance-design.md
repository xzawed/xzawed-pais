...
- **`server.ts`**: `OracleRepo` 생성 조건에 `MANAGER_WP_CONFORMANCE` 추가(`pool && (ORACLE_DOR||ORACLE_DRAFT||WP_CONFORMANCE)`)·`wpConformance: config.MANAGER_WP_CONFORMANCE`를 SupervisorConfig 전달·기동 경고 2종(conformance on인데 oracleStore 부재·`visibilityMs < 600_000`).
