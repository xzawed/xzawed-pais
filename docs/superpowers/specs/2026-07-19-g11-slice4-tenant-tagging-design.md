...
| `oracles` | A (writer 1/2만) | decomposition consumer의 `upsertDraft`. ⚠️ **사람 `POST /oracles` 시드가 타는 `OracleRepo.upsert`는 태그 소스가 없다**(C6 — Manager엔 `authUser`가 없어 이 HTTP 경로에서 orgId를 얻을 방법이 없음) → 이 경로로 들어오는 행은 영구 NULL. 코드로 고칠 수 없어 이번 PR 범위에서 제외(문서화만·아래 참고) |
...
#### `oracles`의 두 writer — 하나만 태깅되고, 이는 사소한 갭이 아니다

`oracles`에는 writer가 둘이다. `upsertDraft`(분해 consumer 경로)는 태그되지만, `POST /workflows/:workflowId/oracles` → `oracleRepo.upsert(seed)`가 타는 `upsert`는 `tenant_id`를 컬럼 목록에 아예 넣지 않는다. **이건 이번 PR에서 코드로 고칠 수 없다** — C6(Manager `authHook`은 서비스 토큰만 검증해 `authUser`가 없음) 때문에 이 HTTP 경로에는 태그 소스가 실제로 존재하지 않는다.

이 갭은 부수적이지 않다: `xzawedManager/CLAUDE.md`의 golden freeze 슬라이스가 "golden 초안은 사람 `POST /oracles` 시드"라고 명시하는데, 그 시드가 정확히 이 미태깅 `upsert` 경로를 탄다. 즉 impact 검증 채널(`approvedGoldensForStory`)이 소비하는 golden을 담은 오라클 행이 바로 이 경로로 들어온다. **Slice 4b가 오라클 읽기에 `WHERE tenant_id = ...` 술어를 얹으면**, `approvedGoldensForStory`·`approvedInvariantsForStory`·`approvedOracleForStory`가 이 행들에 대해 null을 반환하고, 그걸 소비하는 conformance/impact/property 채널은 설계상 **null이면 skip(fail-open)** 이므로 검증이 조용히 꺼진다.

**결론**: Slice 4b는 오라클 읽기에 테넌트 술어를 얹기 전에 이 경로의 태그 소스를 먼저 확보해야 한다(예: 라우트에 orgId claim을 전달하는 인증 확장). 이번 PR은 코드를 바꾸지 않고 이 사실을 `docs/LIVE_VS_FLAGGED.md`·`xzawedManager/CLAUDE.md`·이 문서(위 표)에 기록하는 것으로 그친다.
...
| 인덱스 | 질의 0줄 | 4b |
| `oracles` 사람 시드(`POST /oracles`→`upsert`) 태그 소스 확보 | C6(Manager엔 authUser 없음) — 이번 PR의 페이로드-단일 소스 전제로는 코드로 고칠 수 없음(위 "`oracles`의 두 writer" 참고) | Slice 4b 선행 작업(오라클 읽기 술어 전에 해결 필요) |
...
- 단일 사용자 배포의 **런타임 동작은 바이트 동일**하다. 관측 가능 변화는 새 컬럼에 값이 기록된다는 것과, `DecisionRequest`를 읽는 API 응답이 additive로 커진다는 것뿐이다 — `rowToRequest`(`decision.repo.ts:35`)가 이제 `tenantId`를 실어 `GET /projects/:id/decisions/pending`(`api/decision.route.ts:37`)이 그대로 반환한다. 이를 능동적으로 읽어 분기하는 코드는 0줄이며(Orchestrator DecisionsPanel은 미지 필드를 무시), 실동작 영향은 0이다.
