...
| ID | 항목 | 검증 |
|---|---|---|
| O1 | ~~메트릭 수집 코드 0건~~ → **DLQ·PEL 은 `S3.3` 으로 해소**(`GET /metrics`). 고정 키 목록으로는 못 재므로 `SCAN`(`KEYS` 아님)으로 키스페이스를 훑고, 상한에 걸리면 `..._truncated` 로 **합계가 하한임을 드러낸다**. **비용 지표는 남아 있다** | Manager `api/metrics.route.ts` | ◎ |
...
| R2 | **SAST 종단 실증** — cwd ≠ workspaceRoot 조건은 실증했다(별도 디렉토리로 chdir 후 상대경로 호출 → 이슈 1건 검출, 수정 전 0건) | **여전히 부분 해소 — 이 표가 약속한 것이 지켜지지 않았다(2026-08-26 실측).** `S4.3` 이 확정할 거라고 적어 뒀지만, 착륙한 스모크는 **챗 1왕복**이지 감사가 아니다(스크립트에 security·audit 문자열 0건, 왕복 로그도 `plan_task` 까지만). 컨테이너 안에서 SAST 가 실제로 이슈를 내는지는 아직 아무도 안 봤다 |
...
| R12 | 로컬에 pg/redis가 없어 통합 테스트의 CI 실통과를 확인하지 못했다 | **해소(2026-08-26)** — 예측한 경로(`S4.2` 가시화)가 아니라 둘로 닫았다: 임시 컨테이너(pg·redis)로 통합 테스트를 실제로 돌려 실통과를 확인했고, CI 는 `REQUIRE_INTEGRATION=1` 로 게이트가 닫히면 **throw** 한다 — 무음 skip 이 가시화가 아니라 **실패**가 됐다 |
