...
| S4.4 | **교차 서비스 클론 6건 정리 + `cpd` 필수 승격** | — | 10~16 / +150~350 | 별도 |
...
| R13 | **CPD 실패 삼킴을 걷어내니 기존 클론 6건이 드러났다.** 6건 중 5건이 교차 서비스이고, 그중 Manager↔Orchestrator 2건은 두 서비스가 공유 라이브러리를 아예 갖지 않아 저장소의 module-boundaries 규칙 안에서는 제거할 방법이 없다 | **S4.4로 분리** — 승격 전에 아키텍처 결정이 선행된다 |
| R14 | **로컬 CPD 검사가 교차 서비스 클론을 구조적으로 못 본다.** 경로 인자 없이 돌리면 0개 파일을 스캔하고(Windows), 서비스별로 좁혀 돌리면 서비스 간 비교가 성립하지 않는다. 이 저장소의 "0 clones 목표"는 지금까지 한 번도 저장소 전체로 측정된 적이 없다 | **S4.4에서 로컬 실행 방법을 문서화** |
