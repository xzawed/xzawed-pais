...
| P5 | fail-closed 릴리스 게이트(M1·N2)·saga 보상·canary/롤백·워크플로 FSM | ◐ 부분 — P5-1 릴리스 게이트 코어(M1·증거 집계·fail-closed-on-absence·`MANAGER_RELEASE_GATE`·migration 014·#302)+**P5-2a 사인오프**(고아 `gate.blocked`→`degraded_release` DecisionRequest→사람 `accept_known`→`recordSignOff`·`MANAGER_RELEASE_SIGNOFF`·#308)+**P5-2b 배포 게이팅**(릴리스 게이트/사인오프를 `deploy_project` 하드 전제·fail-open-on-absence·`MANAGER_DEPLOY_GATE`·#310)+**P5-3a 강등 모드 FSM 추적(observe-only)**(OPERATIONS_DECISIONS §1 NORMAL/DEGRADED/SAFE 신호 구동 FSM·악화 즉시·호전 히스테리시스·provider open→DEGRADED·budget daily→SAFE·전이 로그 M8·`getMode()` 노출·enforcement 0·`MANAGER_DEGRADED_MODE`·#327) / 잔여: P5-3b 강등 enforcement(SAFE 디스패치 보류·DEGRADED 사인오프)·saga 보상·canary/롤백 | #302, #308, #310, #327 |
| P6 | 의사결정 브리프(§15)·HumanDecision/SignOff 영속(M9)·강등 모드 FSM·관측성/SLO | ◐ M9 영속(migration 011·#288)+**결함 브리프 배선**(lease escalation→defect_brief DecisionRequest·`MANAGER_DECISION_BRIEF`·M9 첫 런타임 소비·#291)+**사람 결정 라우팅**(§11 `fix_reverify` 폐루프: escalated WP 재진입(reopenLease attempt advance→dispatch_signal)·결정 제출 라우트(authHook·IDOR 404)·`MANAGER_DECISION_ROUTING`·#299)+**C0/C1 결정 대기함 UI**(C0 프로젝트 스코프·project_id·migration 015·#303 / Orchestrator 프록시·DecisionsPanel·JWT decidedBy·#306)+**B1 EXPIRED sweep 생산측**(expiresAt 부여+만료 sweep·M8·#312)+**B1 만료 결정 소비자**(`decision.expired`→바운드 재에스컬레이션: `{base}:reesc{n+1}` 새 PENDING 재생성→C1 재노출·상한 `MANAGER_DECISION_REESCALATE_MAX` 소진 시 warn 로그 종단·별도 그룹+group-scoped dedup·`MANAGER_DECISION_EXPIRY`·#322) — 잔여: spec_fix/reject/accept_known 실동작·verification.failed/decomposition.inconsistent 브리프·강등 모드 FSM(N2)·관측성/SLO | #288, #291, #299, #303, #306, #312, #322 |

---

## 진행 예정

post-#286 전면 감사 + post-#312 재감사(7도메인 라이브 대조·적대 검증)가 확정한 슬라이스 순서. **최심 공통 토대부터** 착수 — ~~P6 M9 영속(#288)~~·~~P2r-2 영속(#289)~~·~~P2 간선 추론+epicId(#290)~~·~~P6 결함 브리프(#291)~~·~~P6 fix_reverify 라우팅(#299)~~·~~P5-2 사인오프+배포 게이팅(#308·#310)~~·~~B1 EXPIRED sweep 생산측(#312)~~·~~P2r-3 LLM 생산자(#314)~~·~~P2r-4 라우팅+wp.risk write-back(#316)~~·~~C5 승인 UI(#318)~~·~~D5 §5 모델 라우팅 소비(#320)~~·~~B1 만료 결정 소비자(#322)~~·~~G1 §13 서킷 decompose/advisory 배선(#325)~~·~~P5-3a 강등 모드 FSM 추적 observe-only(#327)~~ 완료. **P2r 가치사슬(생산자→라우팅→UI→모델소비)이 #320로 완성** — 사람 승인 시 wp.risk=HIGH 전이로 mutation θ게이트가 실제 발화. **B1(#322)로 결정 생명주기 폐루프 완성** — 만료 blocking 결정이 바운드 재에스컬레이션으로 C1에 재노출(M8 무음 소실 종결). **G1(#325)로 budget/provider 서킷이 decompose·advisory까지 확장**. **P5-3a(#327)로 운영 강등 모드(NORMAL/DEGRADED/SAFE)를 신호 구동 FSM으로 추적**(observe-only·서킷 신호 집계). 다음(전부 ready_now·prereq 0):

1. **P5-3b 강등 enforcement** `[의존 P5-3a(#327)]`: P5-3a가 추적하는 모드(`getMode()`)를 실제 게이팅에 연결 — SAFE→신규 디스패치 보류(handleDispatch 모드 체크)·DEGRADED→HIGH-risk 사인오프(N2)·SAFE→재개 사인오프. + 추가 신호원(브로커·하트비트·이벤트스토어·보안사고)·stuck-DEGRADED idle-probe.
...
4. **P5-4 saga 보상 · P5-5 canary/롤백** `[의존 P5-3]`: 강등/실패 시 보상 트랜잭션·점진 배포.
