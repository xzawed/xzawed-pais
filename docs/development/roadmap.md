...
| P5 | fail-closed 릴리스 게이트(M1·N2)·saga 보상·canary/롤백·워크플로 FSM | ◐ 부분 — P5-1 릴리스 게이트 코어(M1·증거 집계·fail-closed-on-absence·MANAGER_RELEASE_GATE·migration 014) / 잔여: deploy 게이팅·N2 강등 FSM·saga 보상·canary/롤백 | — |
| P6 | 의사결정 브리프(§15)·HumanDecision/SignOff 영속(M9)·강등 모드 FSM·관측성/SLO | ◐ M9 영속(migration 011)+**결함 브리프 배선**(lease escalation→defect_brief DecisionRequest·`MANAGER_DECISION_BRIEF`·M9 첫 런타임 소비)+**사람 결정 라우팅 부분 착수**(§11 되먹임 — `fix_reverify` 폐루프: escalated WP 재진입(reopenLease attempt advance→dispatch_signal)·결정 제출 라우트(authHook 게이트·IDOR 404)·`MANAGER_DECISION_ROUTING`) — 잔여: spec_fix/reject/accept_known 실동작·verification.failed/decomposition.inconsistent 브리프·EXPIRED sweep·UI 카드·강등 모드 FSM(N2)·관측성/SLO | #288, #291, #299 |
...
1. **P6 사람 결정 라우팅(§11 되먹임)** `[부분 착수 — fix_reverify·의존 #291, P4 4c(#298)]`: `defect_brief` DecisionRequest에 대한 사람 결정(fix_reverify/spec_fix/accept_known/reject)을 §11 계층으로 되먹임(구현/Task/기획 재진입). **fix_reverify 폐루프 착륙**(`MANAGER_DECISION_ROUTING` — `decision.recorded` 소비→escalated WP `reopenLease`(attempt advance)→`dispatch_signal` 재발행으로 구현 계층 재진입·결정 제출 라우트). 라우팅 입력은 **#298 4c가 채운 `defect_brief.context.attribution`(`faultTier`·`counters`) 라벨을 소비** — 어느 계층이 소진됐는지가 라우팅 결정의 근거. 잔여 = spec_fix(재분해)/accept_known(gate override)/reject(saga) 실동작·verification.failed/decomposition.inconsistent 브리프 확장·EXPIRED sweep·UI ESCALATED 카드.
