...
1. **✅ [결정됨 2026-06-05] 언어 런타임 충돌(STACK-CORE)**: 사양 §17 FastAPI/Python vs 현 전 서비스 TS/Node. **결정: "Node 등가로 사양 적응"** — 사양 §0/§18 절차대로 §17을 "Node 등가물(Fastify + 자체 폴링 아웃박스 릴레이; 브로커는 Redis/RabbitMQ 중 §17 재검토)"로 변경 제안하고, 현 TypeScript/Node 자산을 보존한다. 이벤트소싱·아웃박스·게이트를 Node로 구현. → 이후 모든 P0~P6 WP는 Node 스택 전제로 진행.
