...
const CONSUMER_GROUP = 'watcher-consumers'
const CONSUMER_NAME = 'watcher-1'
...
export class Consumer {
  ...
  constructor(
    ...
    private readonly onMessage: (msg: ManagerToWatcherMessage) => Promise<void>,
    private readonly sleep: (ms: number) => Promise<void> = (ms) =>
      new Promise((r) => setTimeout(r, ms)),
  ) {}

  async start(sessionId: string): Promise<void> {
    const stream = `manager:to-watcher:${sessionId}`
  ...
  private async processMessages(stream: string, messages: [string, string[]][]) {
    for (const [msgId, fields] of messages) {
      ...
      const parsed = ManagerToWatcherMessageSchema.safeParse(JSON.parse(raw))
      if (!parsed.success) {
